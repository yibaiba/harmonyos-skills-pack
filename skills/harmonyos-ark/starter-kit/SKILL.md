...
> 模块间存在前置依赖；按以下层级顺序实现可避免返工。

```
Layer 0 (基础，无依赖):
  ├── scaffold/project-structure.md
  ├── scaffold/layer-architecture.md
  └── modules/dark-multi.md          ← 所有 UI 模块的前置

Layer 1 (导航 + 账号):
  ├── modules/tabbar-navigation.md   ← 依赖 Layer 0
  ├── modules/auth-login.md          ← 依赖 Layer 0
  ├── modules/offline-no-login.md    ← 依赖 Layer 0（与 auth-login 二选一）
  └── modules/data-persistence.md    ← 依赖 Layer 0

Layer 2 (内容页面):
  ├── modules/list-page.md           ← 依赖 tabbar-navigation
  ├── modules/detail-page.md         ← 依赖 list-page
  └── modules/form-submit.md         ← 依赖 tabbar-navigation

Layer 3 (能力增强):
  ├── modules/media-camera.md        ← 依赖 form-submit 或 detail-page
  ├── modules/notification-handling.md ← 依赖 auth-login
  ├── modules/websocket-realtime.md  ← 依赖 auth-login
  └── modules/payment-billing.md     ← 依赖 auth-login + detail-page

Layer 4 (账号升级，可选):
  └── modules/optional-login-upgrade.md ← 依赖 offline-no-login + auth-login
```

**规则**：同 Layer 内模块可并行开发，跨 Layer 必须先完成依赖方。
