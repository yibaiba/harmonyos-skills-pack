...
> **编写任何 HarmonyOS ArkTS/ETS 代码前，必须先读取并遵守以下约束文件。**
> 违反语法约束将直接导致编译失败；违反 API 规范将导致运行时异常。

📄 **→ [topics/arkts-coding-rules.md](topics/arkts-coding-rules.md)**

包含：60+ 条 ArkTS 语法禁止项（分 7 组表格）| HarmonyOS API 使用规范（10 条）| ArkUI 动画规范（4 条）| 快速检查清单（10 项）
