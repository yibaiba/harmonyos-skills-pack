...
| 维度 | 短时任务 | 长时任务 | 延迟任务 (WorkScheduler) |
|------|---------|---------|------------------------|
| **典型场景** | 保存数据、上传日志 | 音乐播放、导航、下载 | 定时同步、定期清理 |
| **执行时机** | 应用退后台后立即 | 应用在前台或后台持续 | 系统空闲时调度 |
| **时间限制** | ~3 分钟 | 无（需前台通知） | 系统决定（有配额） |
| **需要前台通知** | ❌ | ✅ 必须 | ❌ |
| **用户可感知** | ❌ | ✅ 状态栏可见 | ❌ |
| **适用 API** | `requestSuspendDelay` | `startBackgroundRunning` | `startWork` |
| **关键限制** | 超时自动终止 | 类型受限（9 种） | 最短间隔 20 分钟 |

> **选择建议**：需要定时执行 → 延迟任务；需要持续运行 → 长时任务；只需短暂收尾 → 短时任务。
