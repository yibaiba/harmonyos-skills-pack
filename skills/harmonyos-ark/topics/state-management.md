...
`@StorageLink` / `@LocalStorageLink` 仅适合绑定基础字段、数组或可序列化快照。不要直接将 `@ObservedV2` 装饰的 class 作为 Storage 装饰器的属性类型，否则会触发 `10905348` 编译错误。页面级 `@ObservedV2` ViewModel 应保持为本地实例，跨页仅同步 `id` / `title` / `count` 等快照字段。
...
> 📄 **完整迁移指南**（决策矩阵 + 代码示例 + 混用规则 + 检查清单）→ [componentv2-migration.md](componentv2-migration.md)
